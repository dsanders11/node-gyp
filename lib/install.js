  ...
  async function copyDirectory (src, dest) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        // file. Use fs.rename for an atomic move so no process ever sees a
        // partially written file. For cross-filesystem (EXDEV), copy to a
        // temp file in the dest directory first, then rename within the
        ...
        try {
          ...
          if (err.code === 'EXDEV') {
            // Cross-filesystem: copy to a uniquely named temp file in the
            // dest directory, then rename into place atomically
            const tmpFile = `${destFile}.tmp.${crypto.randomBytes(6).toString('hex')}`
            try {
              await fs.copyFile(srcFile, tmpFile)
              // On Windows the dest may be locked by another process —
              // wrap the same-volume rename in backOff for contention
              if (win) {
                await backOff(() => fs.rename(tmpFile, destFile))
              } else {
          ...
          } else if (win && (err.code === 'EBUSY' || err.code === 'EPERM')) {
            // Windows: destination may be locked by another process placing
            // the same file — back off and retry
            await backOff(() => fs.rename(srcFile, destFile))
          } else {
            throw err
          }
        }
